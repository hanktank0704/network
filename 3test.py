...
class SimpleTopology(Topo):
    def build(self):
        ...
        self.addLink(s1, s3, cls=TCLink, bw=100000, delay='0.1ms')
        self.addLink(s2, s3, cls=TCLink, bw=100000, delay='0.1ms')

        self.addLink(s3, h5, cls=TCLink, bw=100000, delay='0.1ms')
...
#    net.build()
    ...
    s_bw, c_bw = net.iperf([src, dst], seconds=5)
    info(s_bw)
    print()
    
    src, dst = net.hosts[1], net.hosts[4]
    s_bw, c_bw = net.iperf([src, dst], seconds=5)
    info(s_bw)
    print()
    
    src, dst = net.hosts[2], net.hosts[4]
    s_bw, c_bw = net.iperf([src, dst], seconds=5)
    info(s_bw)
    print()
    
    src, dst = net.hosts[3], net.hosts[4]
    s_bw, c_bw = net.iperf([src, dst], seconds=5)
    info(s_bw)
    print()
    ...
    result = h2.cmd('ping -c 3', "10.0.0.4")
    lines = result.split('\n')
    print("the result of ping is ", lines[-2])
    print()
    #
